...
export class Init1774556853397 implements MigrationInterface {
    name = 'Init1774556853397';
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `CREATE TABLE \`lawsuits\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`case_number\` varchar(255) NOT NULL, \`decision_date\` date NOT NULL, \`classification\` varchar(255) NULL, \`type\` enum ('gugatan', 'permohonan') NOT NULL DEFAULT 'gugatan', \`status\` enum ('DRAFT', 'SUBMITTED_TO_GUGATAN', 'RECEIVED_BY_GUGATAN', 'SUBMITTED_TO_PERMOHONAN', 'RECEIVED_BY_PERMOHONAN', 'SUBMITTED_TO_HUKUM', 'RECEIVED_BY_HUKUM') NOT NULL DEFAULT 'DRAFT', \`submitted_to_gugatan_at\` timestamp NULL, \`received_by_gugatan_at\` timestamp NULL, \`submitted_to_permohonan_at\` timestamp NULL, \`received_by_permohonan_at\` timestamp NULL, \`pbt_date\` date NULL, \`bht_date\` date NULL, \`ikrar_date\` date NULL, \`submitted_to_hukum_at\` timestamp NULL, \`received_by_hukum_at\` timestamp NULL, \`description\` varchar(255) NULL, \`document_classification_id\` varchar(36) NULL, \`pp_id\` varchar(36) NULL, \`panmud_gugatan_id\` varchar(36) NULL, \`panmud_permohonan_id\` varchar(36) NULL, \`panmud_hukum_id\` varchar(36) NULL, \`js_id\` varchar(36) NULL, UNIQUE INDEX \`IDX_929a8ff414f886635b8811c099\` (\`case_number\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `ALTER TABLE \`lawsuits\` ADD CONSTRAINT \`FK_76ded22f750138eab9dbf042ee6\` FOREIGN KEY (\`panmud_permohonan_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
        );
        await queryRunner.query(
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `ALTER TABLE \`lawsuits\` DROP FOREIGN KEY \`FK_76ded22f750138eab9dbf042ee6\``,
        );
        await queryRunner.query(
